...
static void CG_TransitionSnapshot(void) {
  ...
  // clear the currentValid flag for all entities in the existing
  // snapshot
  for (i = 0; i < cg.snap->numEntities; i++) {
  ...
  if (oldFrame) {
    ...
    // if we are not doing client side movement prediction for
...
static snapshot_t *CG_ReadNextSnapshot(void) {
  ...
  if (cg.latestSnapshotNum > cgs.processedSnapshotNum + 1000) {
    CG_Printf("[skipnotify]WARNING: CG_ReadNextSnapshot: way "
              "out of range, %i > %i\n",
  ...
  while (cgs.processedSnapshotNum < cg.latestSnapshotNum) {
    ...
    // FIXME: why would trap_GetSnapshot return a snapshot with
...
void CG_ProcessSnapshots(void) {
  ...
  if (n != cg.latestSnapshotNum) {
    if (n < cg.latestSnapshotNum) {
      ...
      CG_Error("CG_ProcessSnapshots: n < "
  ...
  do {
    ...
    if (!cg.nextSnap) {
      ...
      // if we still don't have a nextframe, we will
      // just have to extrapolate
      if (!snap) {
        break;
      }
      ...
      if (cg.nextSnap->serverTime < cg.snap->serverTime) {
        CG_Error("CG_ProcessSnapshots: Server "
                 "time went backwards");
      }
    }

    // if our time is < nextFrame's, we have a nice
    // interpolating state
  ...
  if (cg.nextSnap != NULL && cg.nextSnap->serverTime <= cg.time) {
    CG_Error("CG_ProcessSnapshots: cg.nextSnap->serverTime <= "
             "cg.time");
  }
}
