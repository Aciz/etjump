...
void CG_MachineGunEjectBrassNew(centity_t *cent) {
  ...
  if (CG_PointContents(re->origin, -1) &
      (CONTENTS_WATER | CONTENTS_SLIME)) //----(SA)	modified since
...
void CG_MachineGunEjectBrass(centity_t *cent) {
  ...
  // JPW NERVE new brass handling behavior because the SP stuff just
  // doesn't cut it for MP
  if (cent->currentState.eFlags & EF_MG42_ACTIVE ||
    ...
    le->angles.trBase[0] = 90; // rand()&31; // JPW NERVE belt-fed rounds should
                               // come out horizontal
    ...
    le->angles.trBase[0] =
        (rand() & 15) + 82;              // bullets should come out
                                         // horizontal not vertical JPW NERVE
    le->angles.trBase[1] = rand() & 255; // random spin from extractor
    le->angles.trBase[2] = rand() & 31;
    le->angles.trDelta[0] = 2;
    le->angles.trDelta[1] = 1;
...
void CG_PyroSmokeTrail(centity_t *ent, const weaponInfo_t *wi) {
  ...
  if (ent->currentState.weapon == WP_LANDMINE) {
    if (ent->currentState.teamNum < 8) {
      ...
      if (!ent->miscTime) {
        ...
        // Arnout: play the armed sound - weird
        // place to do it but saves us sending
        // an event
  ...
  /* smoke pyro works fine in water (well, it's dye in real life, might
     wanna change this in-game) if ( contents & ( CONTENTS_WATER |
     CONTENTS_SLIME | CONTENTS_LAVA ) ) return;
  */
  ...
  for (; t <= ent->trailTime; t += step) {
    ...
    //			CG_ParticleExplosion( "expblue", lastPos,
    // vec3_origin, 100
    ...
    /* this one works
            if (rand()%4)
                CG_ParticleExplosion( "blacksmokeanim", origin,
       dir, 2800+(int)(random()*1500), 15, 45+(int)(rnd*90) );
       // smoke blacksmokeanim else CG_ParticleExplosion(
       "expblue", lastPos, vec3_origin, 100 + (int)(rnd*400), 4,
       4 );	// fire "flare"
    */
  }
}
...
void CG_RocketTrail(centity_t *ent, const weaponInfo_t *wi) {
  ...
  for (; t <= ent->trailTime; t += step) {
    ...
    if (ent->currentState.eType == ET_FLAMEBARREL) {
      ...
      // CG_ParticleExplosion( "twiltb", lastPos,
      // vec3_origin, 300+(int)(rnd*100), 4,
      // 14+(int)(rnd*8) );	// fire
      CG_ParticleExplosion("blacksmokeanim", lastPos, vec3_origin,
                           800 + (int)(rnd * 1500), 5, 12 + (int)(rnd * 30),
                           qfalse); // smoke
  ...
  /*
      ...
      if ((cg.time - ent->lastTrailTime) >= 50 + rand()%50) {
          ent->headJuncIndex = CG_AddSmokeJunc( ent->headJuncIndex,
                                                  ent, // rain - zinx's
     trail fix cgs.media.smokeTrailShader, origin, 4500, 0.4, 20, 80 );
...
static void CG_DynamiteTrail(centity_t *ent, const weaponInfo_t *wi) {
  ...
  if (ent->currentState.teamNum < 4) {
    ...
    trap_R_AddLightToScene(
        origin, 320,
        std::fabs(std::sin((cg.time - ent->currentState.effect1Time) * mult)),
...
static void CG_GrenadeTrail(centity_t *ent, const weaponInfo_t *wi) {
  ...
  for (; t <= ent->trailTime; t += step) {
    ...
    ent->headJuncIndex = CG_AddSmokeJunc(
        ent->headJuncIndex,
...
static qboolean CG_ParseWeaponConfig(const char *filename, weaponInfo_t *wi) {
  ...
  while (1) {
    ...
    /*		if ( !Q_stricmp( token, "whatever_variable" ) )
       { token = COM_Parse( &text_p );	// get the value if (
       !token ) { break;
                }
                continue;
            }*/
  ...
  for (i = 0; i < MAX_WP_ANIMATIONS; i++) {
    ...
    if (newfmt) {
      ...
      token = COM_Parse(&text_p); // barrel hide bits (so objects can
                                  // be flagged to not be drawn during
                                  // all sequences (a reloading hand
                                  // that comes in from off screen for
                                  // that one animation for example)
      if (!token) {
        break;
      }
      wi->weapAnimations[i].moveSpeed |=
...
static qboolean CG_RW_ParseWeaponLinkPart(int handle, weaponInfo_t *weaponInfo,
...
static qboolean CG_RW_ParseWeaponLink(int handle, weaponInfo_t *weaponInfo,
                                      modelViewType_t viewType) {
  pc_token_t token;
...
static qboolean CG_RW_ParseViewType(int handle, weaponInfo_t *weaponInfo,
                                    modelViewType_t viewType) {
  pc_token_t token;
  char filename[MAX_QPATH];
...
void CG_RegisterWeapon(int weaponNum, qboolean force) {
  ...
  switch (weaponNum) {
    ...
    case WP_SMOKE_BOMB:
      ...
      break;
    case WP_MOBILE_MG42_SET: // do we need a seperate file for
                             // this?
    case WP_MOBILE_MG42:
      filename = "mg42.weap";
      break;
    ...
    case WP_PORTAL_GUN:
      filename = "portalgun.weap";
      break; // Feen: PGM
    case WP_ARTY:
      return; // to shut the game up
    default:
      CG_Printf(S_COLOR_RED "WARNING: trying to register weapon %i "
                            "but there "
                            "is no weapon file entry for it.\n",
                weaponNum);
      return;
  }
  ...
  if (!CG_RegisterWeaponFromWeaponFile(va("weapons/%s", filename),
                                       weaponInfo)) {
    CG_Printf(S_COLOR_RED "WARNING: failed to register media "
                          "for weapon %i from %s\n",
              weaponNum, filename);
  }
...
void CG_RegisterItemVisuals(int itemNum) {
  ...
  if (item->icon) {
    ...
    if (item->giType == IT_HOLDABLE) {
      // (SA) register alternate icons (since holdables
      // can have multiple uses, they might have
      // different icons to represent how many uses are
      // left)
...
qboolean CG_GetPartFramesFromWeap(centity_t *cent, refEntity_t *part,
  ...
  if (partid == W_MAX_PARTS) {
    return qtrue; // primary weap model drawn for all frames
                  // right now
  }
  ...
  for (i = 0; i < (cent->pe.weap.animationNumber & ~ANIM_TOGGLEBIT); i++) {
    if (wi->weapAnimations[i].moveSpeed &
        (1 << partid)) // this part has animation for this
                       // sequence
    {
...
static void CG_RunWeapLerpFrame(clientInfo_t *ci, weaponInfo_t *wi,
  ...
  if (!lf->animation) {
    ...
    if ((newAnimation & ~ANIM_TOGGLEBIT) ==
      ...
      CG_ClearWeapLerpFrame(wi, lf,
                            newAnimation); // clear when switching to
                                           // raise (since it should be
                                           // out of view anyway)
    } else {
  ...
  if (cg.time >= lf->frameTime) {
    ...
    if (f >= anim->numFrames) {
      ...
      if (anim->loopFrames) {
        ...
        f = anim->numFrames - 1;
        // the animation is stuck at the end, so
        // it can immediately transition to
        // another sequence
...
void CG_AddPlayerWeapon(refEntity_t *parent, playerState_t *ps,
  ...
  if (BG_IsAkimboWeapon(weaponNum)) {
    if (isPlayer) {
      akimboFire = BG_AkimboFireSequence(
          ...
          cg.predictedPlayerState.ammoclip[BG_FindClipForWeapon(
  ...
  if (!ps) {
    ...
    if ((cent->currentState.eFlags & EF_FIRING) && weapon->firingSound) {
      // lightning gun and guantlet make a different
      // sound when fire is held down
...
static qboolean CG_WeaponHasAmmo(int i) {
  ...
  if (i == WP_KNIFE || i == WP_PLIERS ||
      i == WP_PORTAL_GUN) // Feen: PGM - Portal gun = no ammo
  {
    return qtrue;
  }
...
qboolean CG_WeaponSelectable(int i) {
  ...
  // if holding a melee weapon (chair/shield/etc.) only allow
...
==============
getNextWeapInBank
    Pass in a bank and cycle and this will return the next valid weapon higher
in the cycle. if the weap passed in is above highest in a cycle
...
static int getNextWeapInBank(int bank, int cycle) {
  ...
  if (weapBanksMultiPlayer[bank][cycle]) // return next weapon in bank
                                         // if there is one
  {
...
==============
getPrevWeapInBank
    Pass in a bank and cycle and this will return the next valid weapon lower in
the cycle. if the weap passed in is the lowest in a cycle (0), this will loop
around to the top (MAX_WEAPS_IN_BANK-1) and start down from there looking for a
valid weapon position
==============
*/
...
cycle,
    return that	(colt returns thompson for example) rather than the lowest
weapon
==============
*/
...
cycle,
    return that	(thompson returns colt for example) rather than the highest
weapon
==============
*/
static int getPrevBankWeap(int bank, int cycle, qboolean sameBankPosition) {
  ...
  if (sameBankPosition && weapBanksMultiPlayer[bank][cycle]) {
    ...
    // if it gets to here, no valid weaps in this bank, go down
    // another bank
...
void CG_SetSniperZoom(int lastweap, int newweap) {
  ...
  switch (newweap) {
    ...
    case WP_FG42SCOPE:
      cg.zoomval = cg_zoomDefaultSniper.value; // JPW NERVE changed from
                                               // defaultFG per atvi req
...
void CG_PlaySwitchSound(int lastweap, int newweap) {
  ...
  trap_S_StartSoundVControl(NULL, cg.snap->ps.clientNum, CHAN_WEAPON,
                            switchsound,
...
void CG_FinishWeaponChange(int lastweap, int newweap) {
  ...
  switch (newweap) {
    ...
    case WP_GPG40:
      ...
      //		if(!(cg.pmext.silencedSideArm &
      // 4) && lastweap !=
      // WP_MEDIC_SYRINGE) { newweap = WP_MEDIC_SYRINGE;
      // cg.weaponSelect = newweap;
      //		}
      //		break;
  }
  ...
  // remember which weapon in this bank was last selected so when
  // cycling back to this bank, that weap will be highlighted first
...
void CG_AltWeapon_f(void) {
  ...
  // Arnout: don't allow another weapon switch when we're still swapping
  // the gpg40, to prevent animation breaking
  ...
  if (cg_cycleAllWeaps.integer || !switchBanks) {
    for (i = 0; i < MAX_WEAPS_IN_BANK_MP; i++) {
      ...
      CG_WeaponIndex(num, NULL,
                     &newcycle); // get cycle of new weapon.  if
                                 // it's lower than the original,
                                 // then it cycled around
      ...
      {
        if (num == curweap) // back to start, just
    ...
    for (i = 0; i < MAX_WEAP_BANKS_MP; i++) {
      if (cg_cycleAllWeaps.integer) {
        num = getNextBankWeap(bank + i, cycle,
                              qfalse); // cycling all weaps always
                                       // starts the next bank at
                                       // the bottom
      } else {
      ...
      if (CG_WeaponSelectable(num)) // first entry in bank was
                                    // selectable, no need to scan the
                                    // bank
      {
        break;
      } else {
      ...
      CG_WeaponIndex(num, &newbank,
                     &newcycle); // get the bank of the new weap

      for (j = newcycle; j < MAX_WEAPS_IN_BANK_MP; j++) {
        num = getNextWeapInBank(newbank, j);

        /*				if(num ==
           WP_BINOCULARS) { continue;
                        }*/
      ...
      if (num) // a selectable weapon was found in the
               // current bank
      {
        break;
      }
...
void CG_PrevWeap(qboolean switchBanks) {
  ...
  if (cg_cycleAllWeaps.integer || !switchBanks) {
    for (i = cycle; i >= 0; i--) {
      ...
      CG_WeaponIndex(num, NULL,
                     &newcycle); // get cycle of new weapon.  if
                                 // it's greater than the original,
                                 // then it cycled around
  ...
  //		else: try to find weap in bank that matches cycle
  // position 			else: use base weap in bank
...
void CG_NextWeaponInBank_f(void) {
  ...
  // this cvar is an option that lets the player use his weapon
  // switching keys (probably the mousewheel) for zooming
  // (binocs/snooper/sniper/etc.)
  if (cg.zoomval) {
...
void CG_NextWeapon_f(void) {
  ...
  // Don't try to switch when in the middle of reloading.
  // cheatinfo:	The server actually would let you switch if this check
  // were not
...
void CG_Weapon_f(void) {
  ...
  /*	cg.weaponSelectTime = cg.time;	// flash the current weapon icon
      ...
      CG_WeaponIndex(curweap, &bank, &cycle);		// get bank/cycle
     of current weapon banked = CG_WeaponIndex(num, &newbank,
     ...
     weap's cycle so it will
                                  //	try to initially cycle up to the
     requested weapon
...
void CG_OutOfAmmoChange(qboolean allowforceswitch) {
  ...
  if (allowforceswitch) {
    ...
    // JPW NERVE -- early out if we just dropped dynamite, go to
    // pliers
    ...
    // the parent otherwise, switch to the equivalent if you've
    // got it
    if (cg.weaponSelect >= WP_BEGINSECONDARY &&
        cg.weaponSelect <= WP_LASTSECONDARY) {
      cg.weaponSelect = equiv =
          getAltWeapon(cg.weaponSelect); // base any further changes
                                         // on the parent
      if (CG_WeaponSelectable(equiv))    // the parent was selectable, drop
                                         // back to that
      {
        CG_FinishWeaponChange(cg.predictedPlayerState.weapon,
                              cg.weaponSelect); //----(SA)
  ...
  // didn't have available alternative or equivalent, try another weap
  // in the bank
  CG_WeaponIndex(cg.weaponSelect, &bank,
                 &cycle); // get bank/cycle of current weapon

  // JPW NERVE -- more useful weapon changes -- check if rifle or pistol
  // is still working, and use that if available
  for (i = 0; i < MAX_WEAPS_IN_BANK_MP; i++) {
...
void CG_MG42EFX(centity_t *cent) {
  ...
  for (num = 0; num < cg.snap->numEntities; num++) {
    ...
    if (mg42->currentState.eType == ET_MG42_BARREL &&
      ...
      // AngleVectors (mg42->s.apos.trBase, forward,
      // NULL, NULL);
...
==============
CG_MortarEFX
    Right now mostly copied directly from Raf's MG42 FX, but with the optional
addtion of smoke
==============
*/
...
void CG_FireWeapon(centity_t *cent) {
  ...
  if (cent->currentState.eFlags & EF_MOUNTEDTANK) {
    if (cg_entities
        ...
        8) // should we use a browning?
    {
      trap_S_StartSoundVControl(NULL, cent->currentState.number, CHAN_WEAPON,
  ...
  if (BG_PlayerMounted(cent->currentState.eFlags)) {
    if (cent->currentState.eFlags & EF_AAGUN_ACTIVE) {
      ...
      // cent->currentState.number, CHAN_WEAPON,
      // cgs.media.hflakWeaponSnd );
    } else {
      trap_S_StartSoundVControl(NULL, cent->currentState.number, CHAN_WEAPON,
                                cgs.media.hWeaponSnd,
                                DEFAULT_VOLUME * etj_weaponVolume.value);
  ...
  if ((cent->currentState.event & ~EV_EVENT_BITS) == EV_FIRE_WEAPON_LASTSHOT) {
    ...
    // try to use the lastShotSound, but don't assume it's
    // there. if a weapon without the sound calls it, drop back
    // to regular fire sound
  ...
  {                  // play a sound
    ...
    if (c > 0) {
      c = rand() % c;
      if (firesound[c]) {
        trap_S_StartSoundVControl(NULL, ent->number, CHAN_WEAPON, firesound[c],
                                  DEFAULT_VOLUME * etj_weaponVolume.value);
        ...
        if (fireEchosound && fireEchosound[c]) // check for echo
        {
          ...
          if (gdist > 512 && gdist < 4096) // temp dist.
                                           // TODO: use
                                           // numbers that
                                           // are weapon
                                           // specific
          {                                // use gorg as the new sound
                                           // origin
            VectorMA(cg.refdef_current->vieworg, 64, norm,
                     gorg); // sound-on-a-stick
            trap_S_StartSoundExVControl(
                gorg, ent->number, CHAN_WEAPON, fireEchosound[c], SND_NOCUT,
                DEFAULT_VOLUME * etj_weaponVolume.value);
          }
        }
      }
...
void CG_AddBulletParticles(vec3_t origin, vec3_t dir, int speed, int duration,
  ...
  crandom()*randScale, dir[2] + crandom()*randScale ); VectorScale(
  velocity, (float)speed*3, velocity );
...
void CG_MissileHitWall(int weapon, int clientNum, vec3_t origin, vec3_t dir,
  ...
  switch (weapon) {
    case WP_KNIFE:
      i = rand() % 4;
      if (!surfFlags) {
        sfx = cgs.media.sfx_knifehit[4]; // different values for
                                         // different types
    ...
    case WP_K43_SCOPE:
      ...
      // r = (rand() & 3) + 1; // JPW NERVE increased
      // spark frequency so players can tell where
      // rounds are coming from in MP
      ...
      if (!clientNum) {
        // RF, why is this here? we need sparks
        // if clientNum = 0, used for warzombie
      ...
      } else if (clientNum == 1) // just do a little smoke puff
      {
        ...
        if ((surfFlags & SURF_GRASS || surfFlags & SURF_GRAVEL ||
             surfFlags & SURF_SNOW)) // JPW NERVE added
                                     // SURF_SNOW
...
void CG_MissileHitPlayer(centity_t *cent, int weapon, vec3_t origin, vec3_t dir,
  ...
  switch (weapon) {
    ...
    case WP_KNIFE:
      CG_MissileHitWall(weapon, 0, origin, dir,
                        1); // JPW NERVE this one makes the hitting
                            // fleshy sound. whee
      break;
    default:
      break;
...
void CG_SpawnTracer(int sourceEnt, vec3_t pstart, vec3_t pend) {
  ...
  if (sourceEnt < cgs.maxclients) {
    // for visual purposes, find the actual tag_weapon for this
    // client and offset the start and end accordingly
    if (!(cg_entities[sourceEnt].currentState.eFlags & EF_MG42_ACTIVE ||
  ...
  // subtract the length of the tracer from the end point, so we dont go
...
qboolean CG_CalcMuzzlePoint(int entityNum, vec3_t muzzle) {
  ...
  if (entityNum == cg.snap->ps.clientNum) {
    ...
    if (cg.snap->ps.eFlags & EF_MG42_ACTIVE) {
      ...
      VectorMA(mg42->currentState.pos.trBase, 40, forward,
               muzzle); // wsa -36, made 40 to be in
      ...
      VectorCopy(aagun->lerpOrigin,
                 muzzle); // Gordon: modelindex2 will already
                          // have been incremented on the
                          // server, so work out what it WAS
                          // then
      ...
      if (cg.renderingThirdPerson) {
        ...
        // bani - fix firstperson tank muzzle
        // origin if drawgun is off
        if (!cg_drawGun.integer) {
  ...
  //----(SA)	removed check.  is this still necessary?  (this way
  // works for
  // ai's firing mg42)  should I check for mg42? 	if (
  // !cent->currentValid ) { 		return qfalse;
  //	}
  //----(SA)	end
...
// ydnar: this is silly--the entity is a mg42 barrel, so just use itself
    ...
    for (num = 0; num < cg.snap->numEntities; num++) {
      ...
      if (aagun->currentState.eType == ET_AAGUN &&
        ...
        AngleVectors(cg.snap->ps.viewangles, forward, right, up);
        VectorCopy(aagun->lerpOrigin,
                   muzzle); // Gordon: modelindex2 will
                            // already have been
                            // incremented on the
                            // server, so work out what
                            // it WAS then
        BG_AdjustAAGunMuzzleForBarrel(muzzle, forward, right, up,
                                      (aagun->currentState.modelindex2 + 3) %
                                          4);
      }
    }
  } else {
...
void CG_Bullet(vec3_t end, int sourceEntityNum, vec3_t normal, qboolean flesh,
  ...
  // Arnout: snap tracers for MG42 to viewangle of client when antilag
  // is enabled
