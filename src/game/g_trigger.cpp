...
void Touch_Multi(gentity_t *self, gentity_t *other, trace_t *trace) {
  ...
  if ((self->spawnflags &
      ...
      (other->client->sess.deathrunFlags &
       static_cast<int>(DeathrunFlags::Active)) == 0) {
    return;
  }
...
// trigger savereset ala TJMod
// Atm is bugged->2 players at the same time -> won't reset for both.
// FIXME

void trigger_savereset_touch(gentity_t *self, gentity_t *other,
                             trace_t *trace) {
  if (!other->client) {
    return;
  }
...
only)
"healtotal"		the maximum of healing this trigger can do. if <= 0,
it's unlimited. default 0 (whole numbers only)
...
qboolean G_IsAllowedHeal(gentity_t *ent) {
  ...
  /*	for( i = 0; i < 2; i++ ) {
     ...
     ent->client->lastHealTimes[i] > 60000) ) {
     ent->client->lastHealTimes[i] = level.time; return qtrue;
          }
      }
...
void heal_touch(gentity_t *self, gentity_t *other, trace_t *trace) {
  ...
  for (i = 0; i < clientcount; i++) {
    healvalue = std::min(touchClients[i]->client->ps.stats[STAT_MAX_HEALTH] -
    ...
    G_AddPredictableEvent(other, EV_ITEM_PICKUP,
                          BG_FindItemForClassName("item_health_cabinet") -
                              bg_itemlist);
...
only)
"ammototal"		the maximum clips of ammo this trigger can add. if <= 0,
it's unlimited. default 0 (whole numbers only)
"target"		cabinet that this entity is linked to
*/
...
qboolean G_IsAllowedAmmo(gentity_t *ent) {
  ...
  /*	for( i = 0; i < 2; i++ ) {
     ...
     ent->client->lastAmmoTimes[i] > 60000) ) {
...
void ammo_touch(gentity_t *self, gentity_t *other, trace_t *trace) {
  ...
  if (self->health == -9999) {
    ...
    count = std::min((float)clientcount, self->health / (float)self->damage);
  }
  ...
  for (i = 0; i < count; i++) {
    ...
    if (ammoAdded) {
      ...
      if (self->health != -9999) {
        // reduce the ammount of available ammo
        // by the added clip number
        self->health -= self->damage;
        //				G_Printf("%i
        // clips left\n", self->health
        //);
...
void SP_trigger_aidoor(gentity_t *ent) {
  if (!ent->targetname) {
    G_Printf("trigger_aidoor at loc %s does not have a "
             "targetname for "
...
void Touch_flagonly(gentity_t *ent, gentity_t *other, trace_t *trace) {
  ...
  if (ent->spawnflags & RED_FLAG && other->client->ps.powerups[PW_REDFLAG]) {
    ...
    AddScore(other, ent->accuracy); // JPW NERVE set from map,
                                    // defaults to 20
...
void Touch_flagonly_multiple(gentity_t *ent, gentity_t *other, trace_t *trace) {
  ...
  if (ent->spawnflags & RED_FLAG && other->client->ps.powerups[PW_REDFLAG]) {
    ...
    if (ent->message && strlen(ent->message)) {
      const char *msg =
          findAndReplaceNametags(ent->message, other->client->pers.netname);
...
void SP_trigger_flagonly(gentity_t *ent) {
  ...
  // JPW NERVE -- if this trigger has a "score" field set, then
  // completing an objective
  //  inside of this field will add "score" to the right player team.
  //  storing this in ent->accuracy since that's unused.
...
void Think_SetupObjectiveInfo(gentity_t *ent) {
  ...
  if (ent->target_ent->s.eType == ET_EXPLOSIVE) {
    ...
    constructibles[1] =
        G_FindByTargetname(constructibles[0],
                           ent->target); // see if we are targetting a 2nd one
    ...
    if (constructibles[1]) {
      ...
      if (constructibles[1]->s.eType != ET_CONSTRUCTIBLE) {
        G_Error("'trigger_objective_info' targets "
                "multiple entities with "
                "targetname '%s', the second one "
                "isn't a 'func_constructible'\n",
                ent->target);
      }

      if (team[0] == team[1]) {
        G_Error("'trigger_objective_info' targets "
                "two 'func_constructible' "
                "entities with targetname '%s' "
                "that are constructible by the "
                "same team\n",
                ent->target);
      }
    ...
    if (constructibles[0]->s.angles2[1] != 0) {
      ...
      // Arnout: spawn a constructible icon - this is
      // for compass usage
      gentity_t *e;
      e = G_Spawn();
      ...
      if (constructibles[1]) {
        // see if one of the two is still
        // partially built (happens when a
        // multistage destructible construction
        // blows up for the first time)
...
void SP_trigger_objective_info(gentity_t *ent) {
  ...
  if (ent->spawnflags & MESSAGE_OVERRIDE) {
    if (!ent->spawnitem) {
      G_Error("'trigger_objective_info' has override "
              "flag set but no override "
              "text\n");
    }
  }

  // Gordon: for specifying which commandmap objectives this entity
  // "belongs" to
  ...
  if (level.numOidTriggers >= MAX_OID_TRIGGERS) {
    G_Error("Exceeded maximum number of "
            "'trigger_objective_info' entities\n");
  }

  // JPW NERVE -- if this trigger has a "score" field set, then blowing
  // up an objective
  //  inside of this field will add "score" to the right player team.
  //  storing this in ent->accuracy since that's unused.
  ...
  if (!ent->target) {
    ...
    // Arnout: finalize spawing on fourth frame to allow for
    // proper linking with targets
...
// dust particles
void trigger_concussive_touch(gentity_t *ent, gentity_t *other,
                              trace_t *trace) {
