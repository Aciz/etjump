...
int Pickup_Powerup(gentity_t *ent, gentity_t *other) {
  ...
  if (!other->client->ps.powerups[ent->item->giTag]) {
    ...
    // some powerups are time based on how long the powerup is
    // /used/ rather than timed from when the player picks it
    // up.
    if (ent->item->giTag == PW_NOFATIGUE) {
    } else {
      // round timing to seconds to make multiple
      // powerup timers count in sync
...
==============
*/
int Pickup_Treasure(gentity_t *ent, gentity_t *other) { return -1; }
...
void UseHoldableItem(gentity_t *ent, int item) {
  switch (item) {
    ...
    case HI_WINE: // 1921 Chateu Lafite - gives 25 pts health up
                  // to max health
      ent->health += 25;
    ...
    case HI_WATER: // protection from drowning - 30 sec
    ...
    case HI_STAMINA: // restores fatigue bar and sets
                     // "nofatigue" for a time period (currently
                     // forced to 60 sec)
      //----(SA)	NOTE:	currently only gives
      // free nofatigue time,
      // doesn't reset fatigue bar.
      // (this is because I'd like the restore to be
      // visually gradual (on the HUD item representing
      // current status of your fatigue) rather than
    ...
    case HI_BOOK2:
    case HI_BOOK3:
      // G_AddEvent(	ent, EV_POPUPBOOK, (item -
      // HI_BOOK1)+1 );
      break;
  }
}
              ...
              int outOfReserve) // is the amount to be added out of reserve
{
  ...
  if (ammomove) {
    ...
    ps->ammoclip[BG_FindClipForWeapon(static_cast<weapon_t>(weapon))] +=
        ammomove;
    return qtrue;
  }
  return qfalse;
...
Add_Ammo
    ...
    (like the AI "infinite ammo" where they get below 900 and force back up to
999)
...
int Add_Ammo(gentity_t *ent, int weapon, int count, qboolean fillClip) {
  int ammoweap = BG_FindAmmoForWeapon(static_cast<weapon_t>(weapon));
  int originalCount;
  int maxammo = BG_MaxAmmoForWeapon(static_cast<weapon_t>(ammoweap),
                                    ent->client->sess.skill);
...
int Pickup_Ammo(gentity_t *ent, gentity_t *other) {
  // added some ammo pickups, so I'll use ent->item->quantity if no
  // ent->count
  if (ent->count) {
...
void G_DropWeapon(gentity_t *ent, weapon_t weapon) {
  ...
  if (weapon == WP_KAR98 || weapon == WP_CARBINE) {
    ent2->delay = client->ps.ammo[BG_FindAmmoForWeapon(
        static_cast<weapon_t>(weapAlts[weapon]))];
  } else {
    ent2->delay = 0;
  }

  //	ent2->item->quantity =
  // client->ps.ammoclip[BG_FindClipForWeapon(weapon)]; // Gordon: um,
  // modifying an item is not a good idea
...
int Pickup_Weapon(gentity_t *ent, gentity_t *other) {
  ...
  if (ent->item->giTag == WP_AMMO) {
    ...
    // if LT isn't giving ammo to self or another LT or the
    // enemy, give him some props
    if (other->client->ps.stats[STAT_PLAYER_CLASS] != PC_FIELDOPS) {
      if (ent->parent && ent->parent->client &&
        ...
        // extracted code originally here into
        // AddMagicAmmo -xkan, 9/18/2002 add 1
        // clip of magic ammo for any two-handed
        // weapon
      }
      return RESPAWN_SP;
    }
...
int Pickup_Health(gentity_t *ent, gentity_t *other) {
  ...
  // if medic isn't giving ammo to self or another medic or the enemy,
  // give him some props
...
void Touch_Item_Auto(gentity_t *ent, gentity_t *other, trace_t *trace) {
    ...
    if (ent->item->giTag != WP_AMMO) {
      if (!COM_BitCheck(other->client->ps.weapons, ent->item->giTag)) {
        if (other->client->pers.touchPickupWeapons <= 0) {
          ...
          if (ent->entityOwner != other->s.number &&
              ent->entityOwner != ENTITYNUM_WORLD) {
            return;
          }
        }
      }
  ...
  if (other->client->pers.autoActivate ==
      PICKUP_FORCE) // autoactivate probably forced by the
                    // "Cmd_Activate_f()" function
  {
...
Called by target_give entity and weapon_portalgun_touch to make sure clients are
always given items, regardless of conditions
===============
*/
...
void Touch_Item(gentity_t *ent, gentity_t *other, trace_t *trace) {
  ...
  // play sounds
  if (ent->noise_index) {
    // (SA) a sound was specified in the entity, so play that
    // sound (this G_AddEvent) and send the pickup as
    // "EV_ITEM_PICKUP_QUIET" so it doesn't make the default
    // pickup sound when the pickup event is recieved
  ...
  // ZOID
  // A negative respawn times means to never respawn this item (but
  // don't delete it).  This is used by items that are respawned by
  // third party events such as ctf flags
  if (respawn <= 0) {
    ent->nextthink = 0;
    ent->think = 0;
  } else {
...
void FinishSpawningItem(gentity_t *ent) {
  ...
  ent->entityOwner = ENTITYNUM_WORLD; // Store "world" as owner for
                                      // etj_touchPickupWeapons 1
  ent->s.otherEntityNum2 = 0;         // DHM - Nerve :: takes modelindex2's
  ...
  //----(SA) moved this up so it happens for suspended items too (and
  // made it a
  // function)
  ...
  if (ent->item->giType == IT_HEALTH || ent->item->giType == IT_AMMO) {
    ...
    // having alternate models defined in bg_misc.c for a health
    // or ammo item specify it as "multi-stage" TTimo left-hand
    // operand of comma expression has no effect initial line:
...
void G_RunItem(gentity_t *ent) {
  ...
  // if groundentity has been set to -1, it may have been pushed off an
  // edge
