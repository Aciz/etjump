...
qboolean G_ScriptAction_SetModelFromBrushmodel(gentity_t *ent, char *params) {
  ...
  if (!token[0]) {
    G_Error("G_Scripting: setmodelfrombrushmodel must have an "
            "targetname\n");
  }
...
qboolean G_ScriptAction_ChangeModel(gentity_t *ent, char *params) {
  ...
  token = COM_ParseExt(&pString, qfalse);
  if (!token[0]) {
    G_Error("G_Scripting: changemodel must have a target model "
            "name\n");
  }
...
qboolean G_ScriptAction_ShaderRemap(gentity_t *ent, char *params) {
  ...
  token = COM_ParseExt(&pString, qfalse);
  if (!token[0]) {
    G_Error("G_Scripting: remapshader must have a target "
            "shader name\n");
  }
  ...
  token = COM_ParseExt(&pString, qfalse);
  if (!token[0]) {
    G_Error("G_Scripting: remapshader must have a new shader "
            "name\n");
  }
...
qboolean G_ScriptAction_FollowPath(gentity_t *ent, char *params) {
  ...
  if (params && (ent->scriptStatus.scriptFlags & SCFL_GOING_TO_MARKER)) {
    // we can't process a new movement until the last one has
    // finished
    return qfalse;
  }
  ...
  } else // we have just started this command
  {
    ...
    token = COM_ParseExt(&pString, qfalse);
    if (!token[0]) {
      G_Error("G_Scripting: followpath must have a "
              "direction\n");
    }
    ...
    token = COM_ParseExt(&pString, qfalse);
    if (!token[0]) {
      G_Error("G_Scripting: followpath must have an "
              "targetname\n");
    }
    pSpline = BG_Find_Spline(token);
    if (!pSpline) {
      G_Error("G_Scripting: can't find spline with "
              "\"targetname\" = \"%s\"\n",
              token);
    }
    ...
    if (!wait) {
      ...
      // set the goto flag, so we can keep processing
      // the move until we reach the destination
      ent->scriptStatus.scriptFlags |= SCFL_GOING_TO_MARKER;
...
qboolean G_ScriptAction_StartAnimation(gentity_t *ent, char *params) {
  ...
  token = COM_ParseExt(&pString, qfalse);
  if (!token[0]) {
    G_Error("G_Scripting: startanimation must have a start "
            "frame\n");
  }
  ...
  token = COM_ParseExt(&pString, qfalse);
  if (!token[0]) {
    G_Error("G_Scripting: startanimation must have a frame "
            "count\n");
  }
...
qboolean G_ScriptAction_SetRotation(gentity_t *ent, char *params) {
  ...
  for (i = 0; i < 3; i++) {
    token = COM_Parse(&pString);
    if (!token || !token[0]) {
      G_Error("G_Scripting: syntax: setrotation "
              "<pitchspeed> <yawspeed> "
              "<rollspeed>\n");
    }
...
qboolean G_ScriptAction_FollowSpline(gentity_t *ent, char *params) {
  ...
  } else // we have just started this command
  {
    ...
    token = COM_ParseExt(&pString, qfalse);
    if (!token[0]) {
      G_Error("G_Scripting: followspline must have a "
              "direction\n");
    }
    ...
    if (!Q_stricmp(token, "accum")) {
      ...
      token = COM_ParseExt(&pString, qfalse);
      if (!token[0]) {
        G_Error("G_Scripting: accum without a "
                "buffer index\n");
      }
...
qboolean G_ScriptAction_SetChargeTimeFactor(gentity_t *ent, char *params) {
  ...
  token = COM_ParseExt(&pString, qfalse);
  if (!token[0]) {
    G_Error("G_Scripting: setchargetimefactor must have a "
            "class name\n");
  }
  ...
  if (factor < 0.f) {
    G_Printf("^3WARNING: setchargetimefactor with factor < 0, "
             "clamped to 0\n");
    factor = 0;
  } else if (factor > 1.f) {
    G_Printf("^3WARNING: setchargetimefactor with factor > 1, "
             "clamped to 1\n");
    factor = 1.f;
  }
...
qboolean G_ScriptAction_AllowTankExit(gentity_t *ent, char *params) {
  ...
  token = COM_ParseExt(&pString, qfalse);
  if (!*token) {
    G_Error("G_Scripting: allowtankexit must have a enable "
            "value\n");
  }
...
qboolean G_ScriptAction_AllowTankEnter(gentity_t *ent, char *params) {
  ...
  token = COM_ParseExt(&pString, qfalse);
  if (!*token) {
    G_Error("G_Scripting: allowtankenter must have a enable "
            "value\n");
  }
...
qboolean G_ScriptAction_SetTankAmmo(gentity_t *ent, char *params) {
  ...
  if (!tank) {
    G_Error("G_Scripting: settankammo, failed to find target "
            "(%s)\n",
            token);
  }
...
qboolean G_ScriptAction_AddTankAmmo(gentity_t *ent, char *params) {
  ...
  tank = G_FindByTargetname(NULL, token);
  if (!tank) {
    G_Error("G_Scripting: addtankammo, failed to find target "
            "(%s)\n",
            token);
  }
...
qboolean G_ScriptAction_SetGlobalFog(gentity_t *ent, char *params) {
  ...
  token = COM_ParseExt(&pString, qfalse);
  if (!token[0]) {
    G_Error("G_Scripting: setglobalfog must have a restore "
            "value\n");
  }
  ...
  token = COM_ParseExt(&pString, qfalse);
  if (!token[0]) {
    G_Error("G_Scripting: setglobalfog must have a duration "
            "value\n");
  }
  ...
  token = COM_ParseExt(&pString, qfalse);
  if (!token[0]) {
    G_Error("G_Scripting: setglobalfog without restore flag "
            "must have a depth "
            "for opaque value\n");
  }
...
qboolean G_ScriptAction_GotoMarker(gentity_t *ent, char *params) {
  ...
  } else // we have just started this command
  {
    ...
    token = COM_ParseExt(&pString, qfalse);
    if (!token[0]) {
      G_Error("G_Scripting: gotomarker must have an "
              "targetname\n");
    }
    pPathCorner = BG_Find_PathCorner(token);
    if (pPathCorner) {
      ...
      if (!target) {
        G_Error("G_Scripting: can't find entity "
                "with \"targetname\" = \"%s\"\n",
                token);
      }
    ...
    token = COM_ParseExt(&pString, qfalse);
    if (!token[0]) {
      G_Error("G_Scripting: gotomarker must have a "
              "speed\n");
    }
    ...
    if (ent->s.eType == ET_MOVER) {
      ...
      ent->speed = speed * static_cast<float>(g_moverScale.value);
      ...
      if (trType != TR_LINEAR_STOP) // allow for
...
qboolean G_ScriptAction_Trigger(gentity_t *ent, char *params) {
  ...
  if (!*name) {
    G_Error("G_Scripting: trigger must have a name and an "
            "identifier: %s\n",
            params);
  }
  ...
  if (!Q_stricmp(name, "self")) {
    ...
    // if the script changed, return false so we don't muck with
    // it's variables
    return ((trent != ent) || (oldId == trent->scriptStatus.scriptId)) ? qtrue
                                                                       : qfalse;
...
qboolean G_ScriptAction_MusicStart(gentity_t *ent, char *params) {
  ...
  token = COM_ParseExt(&pString, qfalse);
  if (!token[0]) {
    G_Error("G_Scripting: syntax: mu_start <musicfile> "
            "<fadeuptime>");
  }
...
qboolean G_ScriptAction_MusicPlay(gentity_t *ent, char *params) {
  ...
  token = COM_ParseExt(&pString, qfalse);
  if (!token[0]) {
    G_Error("G_Scripting: syntax: mu_play <musicfile> [fadeup "
            "time]");
  }
  Q_strncpyz(cvarName, token, sizeof(cvarName));
...
qboolean G_ScriptAction_AlertEntity(gentity_t *ent, char *params) {
  ...
  while (1) {
    ...
    if (!alertent) {
      if (!foundalertent) {
        G_Error("G_Scripting: alertentity cannot "
                "find targetname \"%s\"\n",
                params);
      } else {
        break;
    ...
    if (alertent->client) {
      ...
      if (!alertent->AIScript_AlertEntity) {
        G_Error("G_Scripting: alertentity \"%s\" "
                "(classname = %s) doesn't have "
                "an \"AIScript_AlertEntity\" "
                "function\n",
...
qboolean G_ScriptAction_TagConnect(gentity_t *ent, char *params) {
  ...
  if (!parent) {
    ...
    if (!parent) {
      G_Error("G_ScriptAction_TagConnect: unable to "
              "find entity with "
              "targetname \"%s\"",
              token);
    }
  ...
  // clear out the angles so it always starts out facing the tag
  // direction
...
qboolean G_ScriptAction_AxisRespawntime(gentity_t *ent, char *params) {
  ...
  token = COM_Parse(&pString);
  if (!token[0]) {
    G_Error("G_ScriptAction_AxisRespawntime: time parameter "
            "required\n");
  }
...
qboolean G_ScriptAction_AlliedRespawntime(gentity_t *ent, char *params) {
  ...
  token = COM_Parse(&pString);
  if (!token[0]) {
    G_Error("G_ScriptAction_AlliedRespawntime: time parameter "
            "required\n");
  }
...
qboolean G_ScriptAction_NumberofObjectives(gentity_t *ent, char *params) {
  ...
  token = COM_Parse(&pString);
  if (!token[0]) {
    G_Error("G_ScriptAction_NumberofObjectives: number "
            "parameter required\n");
  }
...
qboolean G_ScriptAction_SetDebugLevel(gentity_t *ent, char *params) {
  ...
  if (!params || !params[0]) {
    G_Error("G_ScriptAction_SetDebugLevel: requires integer "
            "level\n");
  }
...
qboolean G_ScriptAction_VoiceAnnounce(gentity_t *ent, char *params) {
  ...
  token = COM_Parse(&pString);
  if (!token[0]) {
    G_Error("G_ScriptAction_VoiceAnnounce: team parameter "
            "required\n");
  }
  ...
  token = COM_Parse(&pString);
  if (!token[0]) {
    G_Error("G_ScriptAction_VoiceAnnounce: system message "
            "parameter required\n");
  }

  sysmsg = G_GetSysMessageNumber(token);
  if (sysmsg == -1) {
    G_Error("G_ScriptAction_VoiceAnnounce: invalid system "
            "message\n");
  }

  G_SendSystemMessage(static_cast<sysMsg_t>(sysmsg),
                      !num ? TEAM_AXIS : TEAM_ALLIES);
...
qboolean G_ScriptAction_SetWinner(gentity_t *ent, char *params) {
  ...
  token = COM_Parse(&pString);
  if (!token[0]) {
    G_Error("G_ScriptAction_SetWinner: number parameter "
            "required\n");
  }
...
qboolean G_ScriptAction_SetDefendingTeam(gentity_t *ent, char *params) {
  ...
  token = COM_Parse(&pString);
  if (!token[0]) {
    G_Error("G_ScriptAction_SetDefendingTeam: number parameter "
            "required\n");
  }
...
qboolean G_ScriptAction_AddTeamVoiceAnnounce(gentity_t *ent, char *params) {
  ...
  token = COM_Parse(&pString);
  if (!*token) {
    G_Error("G_ScriptAction_AddTeamVoiceAnnounce: team "
            "parameter required\n");
  }
  ...
  token = COM_Parse(&pString);
  if (!*token) {
    G_Error("G_ScriptAction_AddTeamVoiceAnnounce: sound "
            "parameter required\n");
  }
...
qboolean G_ScriptAction_RemoveTeamVoiceAnnounce(gentity_t *ent, char *params) {
  ...
  token = COM_Parse(&pString);
  if (!*token) {
    G_Error("G_ScriptAction_RemoveTeamVoiceAnnounce: team "
            "parameter required\n");
  }
  ...
  token = COM_Parse(&pString);
  if (!*token) {
    G_Error("G_ScriptAction_RemoveTeamVoiceAnnounce: sound "
            "parameter required\n");
  }
...
qboolean G_ScriptAction_TeamVoiceAnnounce(gentity_t *ent, char *params) {
  ...
  token = COM_Parse(&pString);
  if (!*token) {
    G_Error("G_ScriptAction_TeamVoiceAnnounce: team parameter "
            "required\n");
  }
  ...
  token = COM_Parse(&pString);
  if (!*token) {
    G_Error("G_ScriptAction_TeamVoiceAnnounce: sound parameter "
            "required\n");
  }
...
qboolean G_ScriptAction_Announce_Icon(gentity_t *ent, char *params) {
  ...
  token = COM_Parse(&pString);
  if (!*token) {
    G_Error("G_ScriptAction_Announce_Icon: statement parameter "
            "required\n");
  }
...
qboolean G_ScriptAction_Announce(gentity_t *ent, char *params) {
  ...
  token = COM_Parse(&pString);
  if (!token[0]) {
    G_Error("G_ScriptAction_Announce: statement parameter "
            "required\n");
  }
...
qboolean G_ScriptAction_SetDamagable(gentity_t *ent, char *params) {
  ...
  Q_strncpyz(name, token, sizeof(name));
  if (!name[0]) {
    G_Error("G_Scripting: setdamagable must have a name and an "
            "state\n");
  }
...
qboolean G_ScriptAction_SetState(gentity_t *ent, char *params) {
  ...
  Q_strncpyz(name, token, sizeof(name));
  if (!*name) {
    G_Error("G_Scripting: setstate must have a name and an "
            "state\n");
  }
  ...
  while (1) {
    ...
    if (!target) {
      if (!found) {
        G_Printf("^1Warning: setstate called "
                 "and no entities found\n");
      }
      break;
    }
...
qboolean G_ScriptAction_StartCam(gentity_t *ent, char *params) {
  ...
  token = COM_ParseExt(&pString, qfalse);
  if (!token[0]) {
    G_Error("G_ScriptAction_StartCam: filename parameter "
            "required\n");
  }
...
qboolean G_ScriptAction_SetInitialCamera(gentity_t *ent, char *params) {
  ...
  token = COM_ParseExt(&pString, qfalse);
  if (!token[0]) {
    G_Error("G_ScriptAction_SetInitialCamera: filename "
            "parameter required\n");
  }
  ...
  token = COM_ParseExt(&pString, qfalse);
  if (!token[0]) {
    G_Error("G_ScriptAction_SetInitialCamera: black parameter "
            "required\n");
  }
...
qboolean G_ScriptAction_RepairMG42(gentity_t *ent, char *params) {
  ...
  // look for entities
  for (target = &g_entities[MAX_CLIENTS - 1]; target;
       target = G_FindByTargetname(target, name)) {
...
qboolean G_ScriptAction_PrintAccum(gentity_t *ent, char *params) {
  ...
  if (!params || !params[0]) {
    G_Error("G_ScriptAction_PrintAccum: syntax: PrintAccum "
            "<accumNumber>\n");
  }
...
qboolean G_ScriptAction_PrintGlobalAccum(gentity_t *ent, char *params) {
  ...
  if (!params || !params[0]) {
    G_Error("G_ScriptAction_PrintGlobalAccum: syntax: "
            "PrintGlobalAccum "
            "<globalAccumNumber>\n");
  }
...
qboolean G_ScriptAction_ConstructibleClass(gentity_t *ent, char *params) {
  ...
  token = COM_ParseExt(&pString, qfalse);
  if (!token) {
    G_Error("G_Scripting: \"constructible_class\" must have a "
            "class value\n");
  }
...
qboolean G_ScriptAction_ConstructibleChargeBarReq(gentity_t *ent,
  ...
  token = COM_ParseExt(&pString, qfalse);
  if (!token) {
    G_Error("G_Scripting: \"constructible_chargebarreq\" must "
            "have a fraction "
            "value\n");
  }
  ...
  if (value < 0) {
    G_Error("G_Scripting: \"constructible_chargebarreq\" has a "
            "bad value %f\n",
            value);
  }
...
qboolean G_ScriptAction_ConstructibleHealth(gentity_t *ent, char *params) {
  ...
  token = COM_ParseExt(&pString, qfalse);
  if (!token) {
    G_Error("G_Scripting: \"constructible_health\" must have a "
            "health value\n");
  }
...
qboolean G_ScriptAction_ConstructibleWeaponclass(gentity_t *ent, char *params) {
  ...
  token = COM_ParseExt(&pString, qfalse);
  if (!token) {
    G_Error("G_Scripting: \"constructible_weaponclass\" must "
            "have a weapon "
            "class value\n");
  }
...
qboolean G_ScriptAction_ConstructibleDuration(gentity_t *ent, char *params) {
  ...
  token = COM_ParseExt(&pString, qfalse);
  if (!token) {
    G_Error("G_Scripting: \"constructible_duration\" must have "
            "a duration value\n");
  }
...
qboolean G_ScriptAction_Cvar(gentity_t *ent, char *params) {
  const char *pString, *token;
  char lastToken[MAX_QPATH], name[MAX_QPATH], cvarName[MAX_QPATH];
  int cvarValue;
  qboolean terminate, found;
