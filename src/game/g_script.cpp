...
g_script_event_define_t gScriptEvents[] = {
    ...
    {"death", NULL},                               // RIP
    {"activate", G_Script_EventMatch_StringEqual}, // something has triggered us
                                                   // [activator team]
    {"stopcam", NULL},
    {"playerstart", NULL},
...
void G_Script_ScriptLoad(void) {
  ...
  if (g_mapScriptDir.string[0]) {
    ...
    Q_strncpyz(filename, g_mapScriptDir.string, sizeof(filename));
  ...
  // END Mad Doc - TDF
  // Arnout: make sure we terminate the script with a '\0' to prevent
  // parser from choking level.scriptEntity = G_Alloc( len );
  // trap_FS_Read( level.scriptEntity, len, f );
  level.scriptEntity = static_cast<char *>(G_Alloc(len + 1));
  trap_FS_Read(level.scriptEntity, len, f);
  *(level.scriptEntity + len) = '\0';

  // Gordon: and make sure ppl haven't put stuff with uppercase in the
  // string table..
  G_Script_EventStringInit();

  // Gordon: discard all the comments NOW, so we dont deal with them
  // inside scripts Gordon: disabling for a sec, wanna check if i can
  // get proper line numbers from error output
...
static void G_Script_ParseSpawnbot(const char **ppScript, char params[],
  ...
  while (token[0]) {
    // if we are currently parsing a spawnbot command, check the
    // parms for a custom character, which we will need to
    // precache on the client
    ...
    if (strrchr(token,
                ' ')) // need to wrap this param in quotes since
...
void G_Script_ScriptChange(gentity_t *ent, int newScriptNum) {
  ...
  // try and run the script, if it doesn't finish, then abort the
  // current script (discard backup)
...
================
*/
int G_Script_GetEventIndex(gentity_t *ent, const char *eventStr,
                           const char *params) {
  int i, eventNum = -1;
  ...
  for (i = 0; gScriptEvents[i].eventStr; i++) {
    if (gScriptEvents[i].hash == hash &&
        !Q_stricmp(eventStr,
                   gScriptEvents[i].eventStr)) // match found
    {
      eventNum = i;
      break;
    }
  ...
  if (eventNum < 0) {
    ...
    {
      G_Printf("devmode-> G_Script_GetEventIndex(), "
               "unknown event: %s\n",
               eventStr);
    }
    return -1;
...
qboolean G_Script_ScriptRun(gentity_t *ent) {
  ...
  while (ent->scriptStatus.scriptStackHead < stack->numItems) {
    ...
    // if the scriptId changed, a new event was triggered in our
    // scripts which did not finish
    ...
    if (g_scriptDebug.integer) {
      if (ent->scriptStatus.scriptStackHead < stack->numItems) {
        G_Printf("%i : (%s) GScript command: %s "
                 "%s\n",
                 level.time, ent->scriptName,
...
void script_linkentity(gentity_t *ent) {
  ...
  // this is required since non-solid brushes need to be linked but not
  // solid
  trap_LinkEntity(ent);
...
void script_mover_blocked(gentity_t *ent, gentity_t *other) {
  // remove it, we must not stop for anything or it will screw up script
  // timing
  ...
  // FIXME: we could have certain entities stop us, thereby "pausing"
  // movement until they move out the way. then we can just call the
  // GotoMarker() again, telling it that we are just now calling it for
  // the first time, so it should start us on our way again
...
void SP_script_multiplayer(gentity_t *ent) {
  ...
  // Gordon: broadcasting this to clients now, should be cheaper in
  // bandwidth for sending landmine info
